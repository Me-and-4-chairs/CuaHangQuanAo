...
window.onload = function () {
    ...
    var tags = ["Manchester", "Arsenal", "Juventus", "Liverpool"];
...
function phanTich_URL_chiTietSanPham() {
    ...
    // Đổi title
    document.title = nameProduct + ' - Shop quần áo thể thao';
    ...
    var s = addThongSo('Chất liệu', sanPhamHienTai.detail.chatlieu);
    s += addThongSo('Form', sanPhamHienTai.detail.form);
    s += addThongSo('Size', sanPhamHienTai.detail.size);
    s += addThongSo('Màu săc', sanPhamHienTai.detail.color);
    s += addThongSo('Thiết kế', sanPhamHienTai.detail.tke);
    s += addThongSo('Đại lý ủy quyền', sanPhamHienTai.detail.dly);
    info.innerHTML = s;
...
function getDetailPromo(sp) {
    switch (sp.promo.name) {
        ...
        case 'moiramat':
            return `Khách hàng sẽ được thử áo miễn phí tại cửa hàng.`;
        ...
        default:
            ...
            return `Cơ hội trúng ` + span + ` khi mua tại cửa hàng`;
    }
}
